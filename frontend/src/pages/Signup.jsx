				...
				<section className='auth-form-panel'>
					<div className='auth-form-heading'><p className='eyebrow'>Create account</p><h2>Start with the basics.</h2><p>Use your email to create a workspace you can grow into.</p></div>
						<form className='auth-form' onSubmit={handleSubmit} autoComplete='on'>
							<div className='auth-field'><label htmlFor='email'>Email address</label><div className='auth-input-wrap'><input ref={emailInputRef} autoFocus={!emailLocked} id='email' name='email' type='email' value={formData.email} onChange={handleChange} onKeyDown={(event) => { if (event.key === 'Enter' && event.currentTarget.checkValidity()) { event.preventDefault(); handleEmailContinue() } }} placeholder='you@example.com' required autoComplete='username' inputMode='email' disabled={emailLocked} />{emailLocked && <button type='button' onClick={handleEditEmail} className='auth-input-action' aria-label='Edit email' title='Edit email'><Pencil size={16} /></button>}</div></div>

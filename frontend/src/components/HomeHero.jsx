...
const HomeHero = () => {
	...
	const [heroColumns, setHeroColumns] = useState([
		{ id: 'backlog', title: 'BACKLOG', tasks: [{ id: 'research', label: 'Research', title: 'Interview five users', meta: 'Today' }, { id: 'planning', label: 'Planning', title: 'Outline launch brief' }] },
		{ id: 'progress', title: 'IN PROGRESS', tasks: [{ id: 'design', label: 'Design', title: 'Polish onboarding flow', meta: '4 subtasks', accent: 'teal' }, { id: 'content', label: 'Content', title: 'Write release notes' }] },
		{ id: 'done', title: 'DONE', tasks: [{ id: 'analytics', label: 'Shipped', title: 'Set up analytics', meta: 'Complete', accent: 'done' }] },
	])

	const moveHeroCard = (targetColumnId) => {
		if (!draggedCard) return
		setHeroColumns((current) => {
			const sourceColumn = current.find((column) => column.tasks.some((task) => task.id === draggedCard.id))
			...
			return current.map((column) => column.id === sourceColumn.id
				? { ...column, tasks: column.tasks.filter((task) => task.id !== draggedCard.id) }
		...
		<section className='hero-section hero-entrance'>
			...
			<div className='hero-board hero-entrance-board' aria-label='Preview of a Kanban board'>
				...
				<div className='hero-board-columns'>
					{heroColumns.map((column) => <div className={`hero-demo-column ${draggedCard ? 'is-drop-ready' : ''}`} key={column.id} onDragOver={(event) => event.preventDefault()} onDrop={() => moveHeroCard(column.id)}><p>{column.title} <b>{column.tasks.length}</b></p>{column.tasks.map((task) => <article className={task.accent === 'teal' ? 'hero-card-teal' : task.accent === 'done' ? 'hero-card-done' : ''} draggable onDragStart={() => setDraggedCard(task)} onDragEnd={() => setDraggedCard(null)} key={task.id}><span>{task.label}</span><strong>{task.title}</strong>{task.meta && <small>{task.accent === 'teal' && <ClipboardList size={12} />}{task.accent === 'done' && <CheckCircle2 size={12} />}{task.meta}</small>}</article>)}</div>)}
				</div>
				<div className='hero-board-footer'><span><Plus size={14} /> Add task</span><span>3 columns · {heroColumns.reduce((total, column) => total + column.tasks.length, 0)} tasks</span></div>
			</div>
		</section>
	)
}
